...
contract TradeRiserToken is Ownable { //ERC - 20 token contract
  ...
  string public constant symbol = "XTI";
  string public constant name = "XTI";
  uint8 public constant decimals = 6;
  uint256 _totalSupply = 500000000 * ((uint)(10)**(uint)(decimals));
  ...
  bool public isLockActiove = true;
  
  function changeLockTransfer (bool _request) public onlyOwner {
    require (isLockActiove);
    locked = _request;
  }

  function finalUnlockTransfer () public onlyOwner {
    require (isLockActiove);
    locked = false;
    isLockActiove = false;
  }
  ...
  constructor() public {
    ...
    tokenHolder = 0x75B2eBbBeFa94a206a8B15ca9efBa2e314031F77;
  ...
  uint public crowdsaleBalance = 250000000 * ((uint)(10)**(uint)(decimals));
  ...
  uint public teamBalance = 125000000 * ((uint)(10)**(uint)(decimals));
  uint public bountyBalance = 25000000 * ((uint)(10)**(uint)(decimals));
  uint public otherBalance = 100000000 * ((uint)(10)**(uint)(decimals));

  uint public constant ICO_FINISH = 1534204740; //08/13/2018 11:59:00 PM GMT
  ...
  function sendOtherBalance (address[] _addresses, uint[] _values) external onlyOwner {
    uint buffer = 0;
    for(uint i = 0; i < _addresses.length; i++){
      balances[_addresses[i]] = balances[_addresses[i]].add(_values[i]);
      buffer = buffer.add(_values[i]);
    ...
    otherBalance = otherBalance.sub(buffer);
    balances[tokenHolder] = balances[tokenHolder].sub(buffer);
  }
